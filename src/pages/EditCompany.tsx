...
import { Building2, MapPin, Instagram, Facebook, Music, Users, Save, ArrowLeft, CheckCircle, Phone } from 'lucide-react';
...
const EditCompany: React.FC = () => {
    ...
    const loadCompanyData = async () => {
      ...
      try {
        ...
        if (userDoc.exists() && userDoc.data().companyData) {
          const existingData = userDoc.data().companyData;
          setCompanyData({
            name: existingData.name || '',
            town: existingData.town || '',
            description: existingData.description || '',
            swishNumber: existingData.swishNumber || '',
            instagram: existingData.instagram || '',
            facebook: existingData.facebook || '',
            tiktok: existingData.tiktok || '',
            owners: existingData.owners?.length > 0 ? existingData.owners : ['']
          });
          setOriginalName(existingData.name || '');
        } else {
          // No company data found, redirect to setup
  ...
  const handleInputChange = (field: keyof CompanyData, value: string) => {
    ...
    // Show URL warning if company name is being changed
    if (field === 'name' && value !== originalName) {
      setShowUrlWarning(true);
    } else if (field === 'name' && value === originalName) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Clean up empty social media fields and update slug if name changed
      ...
      // Check if slug already exists (only if name changed)
      let finalSlug = baseSlug;
      if (companyData.name !== originalName) {
        try {
      ...
      // Get the old slug for cleanup
      const userDoc = await getDoc(doc(db, 'users', currentUser.uid));
      const oldSlug = userDoc.exists() ? userDoc.data().companyData?.slug : null;
      ...
      // Save to Firestore with old slug for redirects
      const updateData: any = {
        companyData: cleanData,
        updatedAt: new Date()
      };

      // If slug changed, store the old slug for redirects
      if (oldSlug && oldSlug !== finalSlug) {
        updateData.oldSlugs = [oldSlug];
      }
      ...
      setError('Kunde inte uppdatera företagsinformation. Försök igen.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Laddar företagsinformation...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Uppdatera information om ditt UF-företag
          </p>
        </div>
        ...
        <div className="bg-white shadow-xl rounded-xl p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {showUrlWarning && (
                <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm text-yellow-700">
                    <strong>OBS:</strong> Att ändra företagsnamnet kommer också att ändra din webbplats URL.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-gray-900">Sociala medier</h3>
